...
# matrix with zeros
matrix_sp <- matrix(0, nrow = n_species, ncol = n_species)

rownames(matrix_sp) <- rownames(sp_mass)
colnames(matrix_sp) <- rownames(sp_mass)

# fill matrix with probabilities
for (i in 1:n_species) {
  for (j in 1:n_species) {
    # probability of species i eating species j
    matrix_sp[i, j] <- probEat(alpha, beta, gamma, sp_mass[i, "f"], sp_mass[j, "f"])
  }
}
...
# set herbivore interaction to 0
matrix_sp <- sweep(matrix_sp, 1, carnivores, FUN = "*")
matrix_sp <- sweep(matrix_sp, 2, carnivores, FUN = "*")

# print sums to check the results
print(rowSums(matrix_sp))
print(colSums(matrix_sp))
```
...
# just second time period
present_species <- sp_occ[ ,2] 
names(present_species) <- rownames(sp_occ)  # species names as row names

# zero out rows where species is not present
matrix_filtered <- sweep(matrix_sp, 1, present_species, FUN = "*" )  # zero out rows
matrix_filtered <- sweep(matrix_filtered, 2, present_species, FUN = "*" )  # zero out columns

# remove rows and columns with no interactions
matrix_cleaned <- empty(matrix_filtered)

# Check to see that the number of rows in your network with probabilities > 0 is 36
print(nrow(matrix_cleaned))  # 36?
print(sum(sp_occ[,2]))  # Check against the sum of present species (36)
...
for (t in 1:n_time_periods) {
    ...
    # species presence in time period t
    present_species <- sp_occ[, t]
    names(present_species) <- rownames(sp_occ)

    # Zero out extinct species in rows and columns 
    matrix_filtered <- sweep(matrix_sp, 1, present_species, FUN = "*")  
...
assign_colors <- function(graph, groups) {
    V(graph)$color <- ifelse(groups[V(graph)$name] == 1, "red", "green")
    return(graph)
}

igraph_networks_colored <- lapply(igraph_networks, function(graph) {
    assign_colors(graph, carnivores)
})

## plot the network
plot(igraph_networks_colored[[1]],
     vertex.size = 10,
...
```{r plot-g3}
#plot network 3
plot(igraph_networks_colored[[5]],
     vertex.size = 10,
     vertex.label.cex = 0.8,
     edge.arrow.size = 0.5,
     main = "Network 5")
