...
OBA_data <- OBA_data %>% select(Genus, Species, Dec..Lat., Dec..Long., Associated.plant)
OBA_data
```
...
OBA_data_xy <- OBA_data %>% 
  select(Dec..Lat., Dec..Long., Associated.plant, GenusSpecies) %>%
...
crs(ecoregions_shp, proj = TRUE) == crs(OBA_data_xy, proj = TRUE)
...
ggplot() +
  ...
  #bee point data
  geom_sf(data = OBA_data_xy, aes(color = Associated.plant) , size = 1) +
